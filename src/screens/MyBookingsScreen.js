...
const MyBookingsScreen = ({ navigation, route }) => {
  ...
  const loadBookings = async () => {
    ...
    try {
        ...
        if (stored) {
          ...
          if (Array.isArray(parsed)) {
            localOnlyBookings = parsed
              .filter(b => !apiBookingIds.has(b.id))
              .map(b => ({
                ...b,
                // Fill in service_name from API data if the local copy is missing it
                service_name: b.service_name || serviceNameByServiceId[b.service_id] || '',
              }));
          }
        }
      } catch (error) {
      ...
      try {
        const enqRes = await getMyEnquiries();
  ...
  const OngoingTab = () => (
    <FlatList
      ...
      keyExtractor={(item) => item.id || item.booking_number}
      refreshControl={
